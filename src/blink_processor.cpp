...
#include <uvdar_robofly/AMIDataForLogging.h>
#include <uvdar_robofly/AMISeqVariables.h>
#include <uvdar_robofly/AMIAllSequences.h>
#include <uvdar_robofly/AMISeqPoint.h>
#include <mutex>
#include <thread>
#include <atomic>
#include <fstream>
...
namespace uvdar{
  ...
  class UVDARBlinkProcessor : public nodelet::Nodelet {
    ...
    private:
      ...
      */
      void insertPoints(const uvdar_robofly::ImagePointsWithFloatStampedConstPtr &, const size_t &);
      ...
      */
      void insertSunPoints(const uvdar_robofly::ImagePointsWithFloatStampedConstPtr&, const size_t&);
      ...
      */
      ...
      using points_seen_callback_t = boost::function<void (const uvdar_robofly::ImagePointsWithFloatStampedConstPtr&)>;
  ...
  void UVDARBlinkProcessor::setupCallbackAndPublisher() {
    ...
    for (size_t i = 0; i < _points_seen_topics_.size(); ++i){
      ...
      points_seen_callback_t callback = [image_index = i, this](const uvdar_robofly::ImagePointsWithFloatStampedConstPtr &points_msg){
      ...
      points_seen_callback_t sun_callback = [image_index=i,this] (const uvdar_robofly::ImagePointsWithFloatStampedConstPtr& sun_points_msg){
    ...
    for (size_t i = 0; i < _blinkers_seen_topics_.size(); ++i) {
      pub_blinkers_seen_.push_back(nh_.advertise<uvdar_robofly::ImagePointsWithFloatStamped>(_blinkers_seen_topics_[i], 1));
      ...
      if(_pub_tracking_stats_){
        if(!_use_4DHT_){
          pub_ami_logging_.push_back(nh_.advertise<uvdar_robofly::AMIDataForLogging>(_ami_logging_topics_[i], 1));
          pub_AMI_all_seq_info.push_back(nh_.advertise<uvdar_robofly::AMIAllSequences>(_ami_all_seq_info_topics[i], 1));
        }
      }
    }
  }
  ...
  void UVDARBlinkProcessor::insertPoints(const uvdar_robofly::ImagePointsWithFloatStampedConstPtr &pts_msg, const size_t & img_index) {
    ...
    uvdar_robofly::AMIDataForLogging ami_logging_msg;
    uvdar_robofly::AMIAllSequences ami_all_seq_msg;
    ros::Time local_last_sample_time = blink_data_[img_index].last_sample_time;
    {
      ...
      for (auto& signal : blink_data_[img_index].retrieved_blinkers) {
        ...
        uvdar_robofly::AMISeqVariables ami_seq_msg;
        ...
        for(auto point_state : *signal.first){
          uvdar_robofly::AMISeqPoint ps_msg;
          uvdar_robofly::Point2DWithFloat p;
      ...
      if(_pub_tracking_stats_){
        ...
        if( _loaded_var_pub_rate_ < diff ) {
          ...
          uvdar_robofly::Point2DWithFloat max_px_shift;
  ...
  void UVDARBlinkProcessor::insertSunPoints(const uvdar_robofly::ImagePointsWithFloatStampedConstPtr& msg, const size_t & image_index) {
    if (!initialized_) return;
